...
class LatentActionQuantization(nn.Module):
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        first_tokens, last_tokens = self.encode(tokens)  # causal transformer
        ...
        tokens, perplexity, codebook_usage, indices, q4inference, q4inference1 = self.vq(first_tokens, last_tokens, codebook_training_only = False)

        tokens4align = q4inference
        # tokenstest = tokens
        ...
        if return_only_codebook_ids:
            return indices, self.vq.codebooks
        ...
        tokens4align = rearrange(tokens4align, 'b (t h w) d -> b t h w d', h=action_h, w=action_w)
        recon_video4inf = self.decode(concat_tokens, tokens4align)
        returned_recon4inf = rearrange(recon_video4inf, 'b c 1 h w -> b c h w')
        ...
        # loss_con = self.codebook_decorrelation_loss(self.vq.codebooks)
        # recon_loss += loss_con

        return recon_loss, num_unique_indices, q4inference, indices, returned_recon, returned_recon4inf
