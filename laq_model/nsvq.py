...
class NSVQ(torch.nn.Module):
    def __init__(self, dim, num_embeddings, embedding_dim, device=torch.device('cpu'), discarding_threshold=0.1, initialization='normal', code_seq_len=1, patch_size=32, image_size = 256):
        ...
        elif code_seq_len == 25:
            self.cnn_encoder = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=3, stride=2,
                                padding=1),  # 16x16 -> 8x8
                torch.nn.ReLU(),
                torch.nn.Conv2d(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=4, stride=1,
                                padding=0),  # 8x8 -> 4x4
            )
    ...
    def init_reduced_codebook(self):
        from sklearn.cluster import KMeans
        # 转为 numpy，方便 sklearn 处理
        codebook_np = self.codebooks.detach().cpu().numpy()
        # 聚类成 8 类
        kmeans = KMeans(n_clusters=6, random_state=42, n_init='auto')
        cluster_labels = kmeans.fit_predict(codebook_np)  # 每个向量的聚类编号，长度 32
        cluster_centers = kmeans.cluster_centers_  # 8 个聚类中心，形状 [8, 32]

        # 如果你需要结果转回 torch：
        cluster_centers_tensor = torch.tensor(cluster_centers, dtype=self.codebooks.dtype, requires_grad=True).to(self.codebooks.device)
        self.codebooks = nn.Parameter(cluster_centers_tensor)

        self.codebooks_used = torch.zeros(self.num_embeddings, dtype=torch.int32, device=self.codebooks.device)
    ...
    def forward(self, input_data_first, input_data_last, codebook_training_only=False):
        ...
        input_data_first = self.encode(input_data_first, batch_size)  # b * d^2 * 32
        input_data_last = self.encode(input_data_last, batch_size)  # b * d^2 * 32
        ...
        #
        # # 如果你需要结果转回 torch：
        # cluster_centers_tensor = torch.tensor(cluster_centers, dtype=self.codebooks.dtype).to(input_data.device)
        # # cluster_labels_tensor = torch.tensor(cluster_labels, dtype=torch.long)
        #
        # # print(cluster_labels)
        #
        # distances = (torch.sum(input_data ** 2, dim=1, keepdim=True)
        #              - 2 * (torch.matmul(input_data, cluster_centers_tensor.t()))
        #              + torch.sum(cluster_centers_tensor.t() ** 2, dim=0, keepdim=True))
        # ###################################################################
        ...
        if codebook_training_only:
            ...
            quantized_input = input_data + vq_error  # 让vq_error越小越好，也就是让codebook与真实差异越像越好
        ...
        # q4inference1 = hard_quantized_input.reshape(batch_size, self.embedding_dim, -1)  # b * 32 * d^2
        # q4inference1 = q4inference1.permute(0, 2, 1).contiguous()  # b * 64 * 32
        q4inference = self.decode(hard_quantized_input, batch_size)

        return quantized_input, perplexity, self.codebooks_used.cpu().numpy(), min_indices.reshape(batch_size, -1), q4inference, None

    def codebook_decorrelation_loss(self, codebook):
        # codebook: [N, D]  -> N个code向量，每个D维
        ...
        loss = off_diag_sim.pow(2).sum() / (codebook.size(0) ** 2)
        return loss
